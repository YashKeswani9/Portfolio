...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-black font-sans flex flex-col relative">
      ...
      <section
        id="home"
        ref={heroRef}
        className=" scroll-mt-24 md:scroll-mt-28 flex flex-col items-center justify-center text-center text-light p-6 md:p-12 h-screen overflow-hidden relative"
      >
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center">
          ...
          >
            {/* Heading */}
            <h1 className="font-heading text-6xl md:text-7xl lg:text-8xl font-bold leading-tight text-light">
  ...
  id="about"
  ref={aboutRef}
  className="relative bg-[#161b22] text-[#e2e8f0] py-20 md:py-32 overflow-hidden"
>
  {/* Top gradient divider for section break */}
  ...
  <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    >
      ...
      <p className="text-xl font-subheading leading-relaxed tracking-wide text-[#e2e8f0]/80">
      </p>
      <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-start mt-4 pt-10">
        {/* Profile Image, with border and shadow on dark */}
        <div className="w-full md:w-1/3 flex justify-center">
          <Image
            ...
            width={300}
            height={300}
            className="rounded-full object-cover w-[200px] h-[200px] md:w-[250px] md:h-[250px] lg:w-[300px] lg:h-[300px] border-4 border-[#23272e] shadow-lg"
          />
        </div>
        {/* Description and (optionally) Skills */}
        <div className="w-full md:w-2/3 flex flex-col gap-8">
          <div>
            <p className="text-xl leading-relaxed tracking-wide text-[#e2e8f0]/90 text-left">
              {siteText.about.description}
            </p>
          </div>
          {/* Optionally skills grid/components here */}
        </div>
      </div>
    </motion.div>
  </div>
    ...
    >
      {projects.map((project) => (
        <motion.div
          key={project.id}
          variants={item}
          className="bg-dark-card rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 border border-dark-border transform hover:scale-105 h-full flex flex-col"
        >
          <div className="p-6 flex flex-col flex-1">
            <div className="mb-4">
              <h3 className="text-xl font-semibold text-light font-body">{project.title}</h3>
            </div>
            <p className="text-light/80 mb-4 flex-1">{project.description}</p>
